...
export default function WhatsAppFloating() {
  ...
  return (
    <div
      className={`fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-50 flex items-center gap-3 transition-all duration-500 ease-out ${
    ...
    >
      {/* Sliding text label */}
      <div
        className={`bg-slate-900/90 backdrop-blur-md text-white text-xs font-bold px-3 py-2 rounded-xl border border-white/10 shadow-lg pointer-events-none transition-all duration-300 transform ${
          isHovered
            ? "opacity-100 translate-x-0 scale-100"
            : "opacity-0 translate-x-4 scale-95"
        } hidden sm:block font-[var(--font-quicksand)]`}
      >
        Chat WhatsApp Kami
      </div>
      ...
      >
        ...
        {/* WhatsApp SVG Icon */}
        <svg className="h-7 w-7 fill-current relative z-10" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
